...
export const monitorHandlers: RequestHandler[] = [
  ...
  http.get("/api/metrics", async () => {
    ...
    const metrics: SystemMetric[] = [
      ...
      {
        name: "内存使用率",
        value: faker.number.float({ min: 40, max: 90, fractionDigits: 1 }),
        unit: "%",
        timestamp: now,
      },
      {
        name: "磁盘使用率",
        value: faker.number.float({ min: 30, max: 85, fractionDigits: 1 }),
        unit: "%",
        timestamp: now,
      },
      {
        name: "网络入流量",
        value: faker.number.float({ min: 10, max: 500, fractionDigits: 0 }),
        unit: "Mbps",
        timestamp: now,
      },
      {
        name: "网络出流量",
        value: faker.number.float({ min: 5, max: 300, fractionDigits: 0 }),
        unit: "Mbps",
        timestamp: now,
      },
      {
        name: "JVM 堆内存",
        value: faker.number.float({ min: 1, max: 8, fractionDigits: 1 }),
        unit: "GB",
        timestamp: now,
      },
    ];
